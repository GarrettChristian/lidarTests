...
def create_model():
  ...
  # Decoder 
  model.add(layers.Dense(16384,activation='sigmoid'))
  model.add(layers.Reshape((2, 32, 256)))
  ...
  model.add(layers.Conv2D(filters=96, kernel_size=(11, 11), strides=(1, 1), activation='relu', padding='same'))
  model.add(layers.BatchNormalization())
  model.add(layers.UpSampling2D(size=(2, 2)))  
...
def main():
  ...
  # history = autoencoder.fit(training_generator, validation_data=validation_generator, epochs=200)
  history = autoencoder.fit(training_generator, validation_data=validation_generator, epochs=200, use_multiprocessing=True, workers=6)
