...
def create_model():
  ...
  # Encoder
  inputs = keras.Input(shape=(32, 512, 1))
  x = inputs
  ...
  # Decoder
  x = layers.Reshape((8, 128, 8))(x)


  x = layers.Conv2D(8, 3, strides=1, activation='relu', padding="same")(x)
  ...
  # # Decoder 
  # model.add(layers.Dense(2048,activation='sigmoid'))
  # model.add(layers.Reshape((4, 64, 8)))
  ...
  # model.add(layers.UpSampling2D((2, 2)))
  # model.add(layers.Conv2D(filters=32, kernel_size=(5, 5), strides=(1, 1), activation='relu', padding='same')) # orig 16
  # model.add(layers.UpSampling2D((2, 2)))
