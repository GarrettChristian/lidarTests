...
def main():
    ...
    x = layers.Conv2D(8, 5, activation="relu", strides=1, padding="same")(encoder_inputs)
    x = layers.MaxPooling2D(2, padding='same')(x)
    x = layers.Conv2D(8, 3, activation="relu", strides=1, padding="same")(x)
    x = layers.MaxPooling2D(2, padding='same')(x)
    x = layers.Conv2D(8, 3, activation="relu", strides=1, padding="same")(x)
    x = layers.MaxPooling2D(2, padding='same')(x)
    x = layers.Conv2D(8, 3, activation="relu", strides=1, padding="same")(x)
    ...
    # x = layers.Reshape((8, 128, 32))(x)
    #x = layers.Reshape((4, 64, 64))(x)
    # x = layers.Reshape((1, 16, 64))(x)
    x = layers.Reshape((2, 32, 8))(x)
    x = layers.Conv2D(8, 3, activation="relu", strides=1, padding="same")(x)
    ...
    x = layers.Conv2D(8, 3, activation="relu", strides=1, padding="same")(x)
    x = layers.UpSampling2D(2)(x)
    # x = layers.Conv2D(8, 5, activation="relu", strides=1, padding="same")(x)
    #x = layers.UpSampling2D(2)(x)
