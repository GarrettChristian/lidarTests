...
def create_model():
  ...
  # Encoder
  model.add(layers.Input(shape=(256, 256, 32, 1)))
  model.add(layers.Conv3D(filters=64, kernel_size=3, strides=1, activation='relu', padding='same')) # orig 16
  ...
  # Decoder 
  model.add(layers.Dense(4096,activation='sigmoid'))
  model.add(layers.Reshape((32, 32, 4, 1)))
  model.add(layers.Conv3D(filters=8, kernel_size=3, strides=1, activation='relu', padding='same')) # orig 8
